...
export class CaixaAdapter implements BankAdapter {
  ...
  parseRow(row: string[], headers: string[]): ParsedTransaction | null {
    try {
      ...
      // Formato Caixa: DD/MM/YYYY
      const date = this.parseDate(dateStr)
      if (!date) {
        return null
      }

      // Formato Caixa: 1.234,56 (positivo entrada, negativo saída)
  ...
  private parseAmount(amountStr: string): number | null {
    // Formato Caixa: 1.234,56 (ponto para milhares, vírgula para decimais)
