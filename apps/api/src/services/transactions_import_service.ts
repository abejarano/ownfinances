...
import { notifyImportCompleted } from "../http/websocket/websocket_handler";
...
export class TransactionsImportService {
  ...
  async process(
    ...
    return { jobId: importJob.getImportJobId() };
  }
      ...
      for (let i = 1; i < rows.length; i++) {
        try {
          ...
          const transaction = Transaction.create({
            ...
            categoryId: null,
            fromAccountId:
              parsed.type === "expense" ? accountId : null,
            toAccountId:
              parsed.type === "income" ? accountId : null,
            note: parsed.note,
            tags: null,
