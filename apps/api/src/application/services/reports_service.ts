import { Criteria, Filters, Operator, Order, FilterInputValue } from "@abejarano/ts-mongodb-criteria";
...
export class ReportsService {
  ...
  async summary(userId: string, period: BudgetPeriodType, date: Date) {
    ...
    const transactions = await this.transactions.search(txCriteria);
    ...
    for (const tx of transactions) {
      ...
      actualByCategory.set(
        tx.categoryId,
        (actualByCategory.get(tx.categoryId) ?? 0) + tx.amount,
      );
    }
